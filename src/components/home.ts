...
const container = `
		<div class="home__left">
			...
			<ul class="socials">
				${SOCIAL_LINKS.map(({ name, url, icon, label, type }) => `
					<li>
						<a href="${type === 'email' ? `mailto:${url}` : type === 'phone' ? `tel:${url}` : url}" target="_blank" rel="noopener" aria-label="${name}">
							${icon}
							<span>${label}</span>
						</a>
					</li>`).join('')
