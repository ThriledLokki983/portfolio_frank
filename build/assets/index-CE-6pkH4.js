(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const t of n)if(t.type==="childList")for(const i of t.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function a(n){const t={};return n.integrity&&(t.integrity=n.integrity),n.referrerPolicy&&(t.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?t.credentials="include":n.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function o(n){if(n.ep)return;n.ep=!0;const t=a(n);fetch(n.href,t)}})();const l=["Home","Research","Teachings","Talks","Contact"],c=`
	<nav id="navigation" class="nav">
		<ul class="nav__list">
			${l.map((r,e)=>`
				<li class="nav__item">
					<a href="${r.toLowerCase()}" aria-selected="${e===0}" class="nav__link">${r}</a>
				</li>`).join("")}
		</ul>
		<button
...
`,u=`
		<div class="home__left">
			...
			<ul class="socials">
				${g.map(({name:r,url:e,icon:a,label:o,type:n})=>`
					<li>
						<a href="${n==="email"?`mailto:${e}`:n==="phone"?`tel:${e}`:e}" target="_blank" rel="noopener" aria-label="${r}">
							${a}
							<span>${o}</span>
						</a>
					</li>`).join("")}
...
//# sourceMappingURL=index-CE-6pkH4.js.map
